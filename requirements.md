...
> ⚠️ **2026-06-16 设计变更（per-user agent_token）**
>
> 本任务书的下文为初版设计（单用户、全局 `AGENT_NOTIFY_TOKEN` + `QQ_TARGET_OPENID`），
> 当前实现已改为**多用户 per-user agent_token 模型**，与下文存在出入。以代码与 README 为准：
>
> - **废弃**：`AGENT_NOTIFY_TOKEN` 与 `QQ_TARGET_OPENID` 两个环境变量。
> - **新增**：每个 QQ 用户通过 `/bind <BIND_SECRET>` 自助注册，机器人私信回复一次性 `agent_token`（`ant_` 前缀）。
> - **鉴权**：`/v1/*` 接口的 `Authorization: Bearer <agent_token>` 由服务端用 `sha256(token)` 查 `targets` 表决定收件人，token 与 QQ 用户一一对应。
> - **DB 变更**：`targets` 表去掉 `name` 的 UNIQUE 约束、`user_openid` 改为唯一；新增 `agent_token_hash`、`token_prefix`、`last_used_at` 三列。
> - **新增命令**：`/unbind` 立即撤销 token，`/whoami` 查看自己的绑定状态。
> - **API 变更**：`/v1/events` 仅返回当前 token 所属用户的事件；`/v1/targets` 改为 `/v1/targets/me`。
> - **错误信息**：401 改为 `invalid or unbound token, send /bind <BIND_SECRET> to the bot first`。
